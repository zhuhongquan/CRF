...
class CRF(object):
    ...
    def create_feature_space(self):
        ...
        self.bigram_scores = np.zeros((len(self.tag_list), len(self.tag_list)))
    ...
    def predict(self, sentence):
        ...
        self.bigram_scores = np.array(
            [[self.get_score(bigram_f) for bigram_f in bigram_feature] for bigram_feature in
        ...
        for i in range(1, length):
            unigram_scores = np.array([self.get_score(self.create_unigram_feature(sentence, i, cur_tag))
                                       for cur_tag in self.tag_list])
            scores = self.bigram_scores + unigram_scores[:, None] + delta[i - 1]  # none用于升维
    ...
    def forward(self, sentence):
        ...
        path_scores[0] = [self.get_score(self.create_feature_template(sentence, 0, self.BOS, cur_tag)) for cur_tag in
                       self.tag_list]
        for i in range(1, len(sentence)):
            unigram_scores = np.array(
                [self.get_score(self.create_unigram_feature(sentence, i, cur_tag)) for cur_tag in self.tag_list])
            scores = self.bigram_scores + unigram_scores[:, None] + path_scores[i - 1]
    ...
    def backward(self, sentence):
        ...
        for i in reversed(range(len(sentence) - 1)):
            unigram_scores = np.array(
                [self.get_score(self.create_unigram_feature(sentence, i + 1, cur_tag)) for cur_tag in self.tag_list])
            scores = self.bigram_scores.T + unigram_scores + path_scores[i + 1]
            path_scores[i] = logsumexp(scores, axis=1)
        return path_scores
    ...
    def update_gradient(self, sentence, tags):
        ...
        for i, tag in enumerate(self.tag_list):
            ...
            p = np.exp(self.get_score(features) + backward_scores[0, i] - dinominator)
            for id in features_id:
                self.g[id] -= p

        for i in range(1, len(sentence)):
            for j, tag in enumerate(self.tag_list):
                ...
                scores = self.bigram_scores[j] + self.get_score(unigram_features)
        ...
        for iter in range(iteration):
            ...
            for i in range(len(self.train_data.sentences)):
                ...
                if b == batch_size:
                    ...
                    self.g = defaultdict(float)
                    self.bigram_scores = np.array([
                        [self.get_score(f) for f in bigram_features]
